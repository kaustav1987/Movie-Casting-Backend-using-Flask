...
AUTH0_DOMAIN = 'kaustavdas.auth0.com'
ALGORITHMS = ['RS256']
...
def check_permissions(permission, payload):
    ...
    if 'permissions' not in payload:
        raise AuthError(
            {'code': 'Bad request header',
             'description': 'Permission not found in header'},
            401)

    if permission not in payload['permissions']:
        raise AuthError(
            {'code': 'Bad request header',
             'description': 'Permission for resource not found in header'},
            401)
...
def verify_decode_jwt(token):
    """
    Verify JWT token fetched from the header
    :param token:
    :return: payload
    """
